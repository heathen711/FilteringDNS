...
HOST, PORT = "192.168.7.11", 53

cache = []
cacheMax = 1000

baseDir = "/share/CACHEDEV1_DATA/homes/admin/DNSlogs/"
...
class MyUDPHandler(SocketServer.BaseRequestHandler):
    def handle(self):
        debug_log("Client: " + self.client_address[0])
        client = ""
        if self.client_address[0].startswith("166."):
            ...
            client = "Sprint"
        elif self.client_address[0].startswith("192.168.7."):
            client = "Local"
        elif self.client_address[0].startswith("10.8.0."):
            client = "Local VPN"
        else:
            debug_log("Invalid client, disconnecting: " + self.client_address[0])
            return 0
        start = time.time()
        data = self.request[0]
    ...
    def DNSQuery(self):
        if "\x07version\x04bind" in self.data.lower():
            ...
            if not self.failed:
                ...
                elif self.domain == "h711.webhop.me" and ( self.askingIP[0].startswith("192.168.7.") or self.askingIP[0].startswith("10.8.0.") ):
                    self.ip = [ "192.168.7.11" ]
                elif self.domain == "heaven.webhop.me" and ( self.askingIP[0].startswith("192.168.7.") or self.askingIP[0].startswith("10.8.0.") ):
                    self.ip = [ "192.168.7.11" ]
